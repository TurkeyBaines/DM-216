package com.dm.game.world.entity.combat.strategy.player.custom;
...
import com.dm.game.world.entity.combat.strategy.basic.MagicStrategy;
...
public class TumekenStrategy extends MagicStrategy<Player> {
    private static final TumekenStrategy INSTANCE = new TumekenStrategy();
    private static CombatProjectile PROJECTILE;

    static {
        try {
            PROJECTILE = CombatProjectile.getDefinition("SHADOW");
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
    ...
    public static TumekenStrategy get() {
        return INSTANCE;
    }
