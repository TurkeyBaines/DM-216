package com.dm.game.world.entity.skill;
...
import com.dm.content.skill.impl.firemaking.Firemaking;
import com.dm.content.skill.impl.fishing.Fishing;
import com.dm.content.skill.impl.fletching.Fletching;
import com.dm.content.skill.impl.herblore.Herblore;
import com.dm.content.skill.impl.hunter.Hunter;
import com.dm.content.skill.impl.mining.Mining;
import com.dm.content.skill.impl.prayer.BoneSacrifice;
import com.dm.content.skill.impl.runecrafting.Runecraft;
import com.dm.content.skill.impl.smithing.Smelting;
import com.dm.content.skill.impl.smithing.Smithing;
import com.dm.content.skill.impl.thieving.Thieving;
import com.dm.content.skill.impl.woodcutting.Woodcutting;
    ...
    private void updateSkill(Player player, int maxLevel, int newMax, Skill skill, boolean levelUp, boolean combat) {
        if (maxLevel < 99 && newMax != maxLevel) {
            ...
            if (levelUp) {
                ...
                if (newMax == 99 && !combat) {
                    World.sendMessage(
                            "<col=7B44B3>Dead Men: <col=" + player.right.getColor() + ">" + player.getName() + " </col>has just reached level 99 in <col=7B44B3>" + Skill.getName(skill.getSkill()) + "</col> with a prestige of " +player.prestige.totalPrestige + "!");
